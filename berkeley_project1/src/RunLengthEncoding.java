...
public class RunLengthEncoding implements Iterable {
	...
	public RunLengthEncoding(int width, int height) {
		this.width = width;
		this.height = height;
		runLength = new ListNode(0);
		runLength.next = new ListNode(width * height);
		runLength.next.prev = runLength;
		// Your solution here.
	}
	...
	public RunLengthEncoding(int width, int height, int[] red, int[] green, int[] blue, int[] runLengths) {
		...
		this.height = height;
		runLength = new ListNode(0);
		runLength.next = new ListNode(red[0], green[0], blue[0], runLengths[0]);
		ListNode tmp = runLength.next;
		for (int i = 1; i < runLengths.length; i++) {
			tmp.next = new ListNode(red[i], green[i], blue[i], runLengths[i]);
			tmp.next.prev = tmp;
			tmp = tmp.next;
		}
		// Your solution here.
	...
	public RunIterator iterator() {
		RunIterator iter = new RunIterator(1, runLength);
		// Replace the following line with your solution.
		return iter;
	...
	public PixImage toPixImage() {
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < data[0]; i++) {
				image.setPixel(count % width, count / width, (short) data[1], (short) data[2], (short) data[3]);
				count++;
			}
		}
		// Replace the following line with your solution.
	...
	public String toString() {
		...
		while (iter.hasNext()) {
			int[] data = iter.next();
			out += "length is " + data[0] + " red pixel is " + data[1] + " green pixel is " + data[2]
					+ " blue pixel is " + data[3] + "\n";
		}
		// Replace the following line with your solution.
		return out;
	}
	...
	public RunLengthEncoding(PixImage image) {
		...
		for (int j = 0; j < height; j++) {
			for (int i = 0; i < width; i++) {
				if (i == 0 && j == 0)
					...
					oldi = ((i + 1) + width * (j) - 2) % width;
					oldj = ((i + 1) + width * (j) - 2) / width;
					if (image.getRed(i, j) == image.getRed(oldi, oldj)
							&& image.getGreen(i, j) == image.getGreen(oldi, oldj)
							&& image.getBlue(i, j) == image.getBlue(oldi, oldj)) {
						length++;
					} else {
						tmp.next = new ListNode(image.getRed(oldi, oldj), image.getGreen(oldi, oldj),
								image.getBlue(oldi, oldj), length);
						tmp.next.prev=tmp;
						length = 1;
						tmp = tmp.next;
					}
		...
		tmp.next = new ListNode(image.getRed(width - 1, height - 1), image.getGreen(width - 1, height - 1),
				image.getBlue(width - 1, height - 1), length);
		tmp.next.prev=tmp;
	...
	public void check() {
		RunIterator iter = iterator();
		int[] dataFirst = iter.next();
		while (iter.hasNext()) {
			int[] dataSecond = iter.next();
			if (dataFirst[0] == dataSecond[0] && dataFirst[1] == dataSecond[1] && dataFirst[2] == dataSecond[2]) {
				System.err.println("the constructor works bad!");
				return;
			}
	...
	public void setPixel(int x, int y, short red, short green, short blue) {
		...
		int length = iter.start.run[0];
		num += length;
		if (num > 1 && num < length) {
			ListNode tmp1 = new ListNode(red, green, blue, 1);
			iter.start.setLength(num - 1);
			ListNode tmp2 = new ListNode(iter.start.run[1], iter.start.run[2], iter.start.run[3], length - num);
			iter.start.next.prev = tmp2;
			tmp2.next = iter.start.next;
			iter.start.next = tmp1;
			tmp1.prev = iter.start;
			tmp1.next = tmp2;
			tmp2.prev = tmp1;
		} else if (num == 1) {
			if (length == 1) {
				if (iter.start.prev.run[0] == 0) {
					if (iter.start.next == null) {
						iter.start.setPixel(red, green, blue);
						return;
					}
					int[] data2 = iter.start.next.run;
					if (data2[1] == red && data2[2] == green && data2[3] == blue) {
						iter.start.next.setLength(iter.start.run[0] + iter.start.next.run[0]);
						iter.start.next.prev=runLength;
						runLength.next=iter.start.next;
					} else {
						iter.start.setPixel(red, green, blue);
					}
				} else if (iter.start.next == null) {
					int[] data1 = iter.start.prev.run;
					if (data1[1] == red && data1[2] == green && data1[3] == blue) {
						iter.start.prev.setLength(iter.start.run[0] + iter.start.prev.run[0]);
						iter.start.prev.next=null;
					}
				} else {
					int[] data1 = iter.start.prev.run;
					int[] data2 = iter.start.next.run;
					if (data1[1] == red && data2[1] == red && data1[2] == green && data2[2] == green && data1[3] == blue
							&& data2[3] == blue) {
						iter.start.prev.setLength(iter.start.run[0] + iter.start.next.run[0] + iter.start.prev.run[0]);
						iter.start.prev.next = iter.start.next.next;
						iter.start.next.next.prev=iter.start.prev;
					} else if (data1[1] == red && data1[2] == green && data1[3] == blue) {
						iter.start.prev.setLength(iter.start.run[0] + iter.start.prev.run[0]);
						iter.start.prev.next = iter.start.next;
						iter.start.next.prev = iter.start.prev;
					} else if (data2[1] == red && data2[2] == green && data2[3] == blue) {
						iter.start.next.setLength(iter.start.run[0] + iter.start.next.run[0]);
						iter.start.prev.next=iter.start.next;
						iter.start.next.prev=iter.start.prev;
					} else {
				...
				if (iter.start.prev.run[0] == 0) {
					ListNode tmp = new ListNode(red, green, blue, 1);
					iter.start.setLength(length - 1);
					tmp.next = iter.start;
					iter.start.prev = tmp;
					runLength.next=tmp;
				} else {
					int[] data = iter.start.prev.run;
					if (data[1] == red && data[2] == green && data[3] == blue) {
						iter.start.prev.setLength(data[0] + 1);
						iter.start.setLength(length - 1);
					} else {
						ListNode tmp = new ListNode(red, green, blue, 1);
						iter.start.setLength(length - 1);
						tmp.next = iter.start;
						tmp.prev = iter.start.prev;
						iter.start.prev.next = tmp;
						iter.start.prev = tmp;
					}
				}
			}
			...
			int[] data = iter.start.next.run;
			if (data[1] == red && data[2] == green && data[3] == blue) {
				iter.start.next.setLength(data[0] + 1);
				iter.start.setLength(length - 1);
			} else {
				ListNode tmp = new ListNode(red, green, blue, 1);
				iter.start.setLength(length - 1);
				tmp.next = iter.start.next;
				tmp.prev = iter.start;
				iter.start.next.prev = tmp;
				iter.start.next = tmp;
			}
		}
