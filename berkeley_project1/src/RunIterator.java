...
public class RunIterator implements Iterator {
	...
	// of
	// the encoding.
	RunIterator(int num, ListNode run) {
		ListNode tmp = run;
		for (int i = 2; i < num; i++) {
			tmp = tmp.next;
		}
	...
	public int[] next() {
		if (start!=null) {
			start = start.next;
			int[] out = start.run;
			// Construct a new array of 4 ints, fill in its values, and return
			// it.
			// Don't forget to advance the RunIterator's pointer so that the
			// next
