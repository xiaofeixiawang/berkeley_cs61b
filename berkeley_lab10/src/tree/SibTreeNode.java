...
class SibTreeNode extends TreeNode {
	...
	public void insertChild(Object item, int c) throws InvalidNodeException {
		if (isValidNode()) {
			...
			SibTreeNode tmp=firstChild;
			while (tmp.nextSibling != null && c > 2) {
				tmp = tmp.nextSibling;
				c--;
			}
			newNode.nextSibling=tmp.nextSibling;
	...
	public void removeLeaf() throws InvalidNodeException {
		if(isValidNode()){
			if(firstChild==null){
				...
				SibTreeNode tmp = parent.firstChild;
				while(tmp.nextSibling!=this)tmp=tmp.nextSibling;
				tmp.nextSibling=nextSibling;
			}
		}else{
			throw new InvalidNodeException();
		}
