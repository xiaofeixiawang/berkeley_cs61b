...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int num=2*horiz*vert-horiz-vert;
    int[] walls=new int[num];
    for(i=0;i<(horiz*(vert-1));i++){
    	walls[i]=i;
    }
    for(i=horiz*(vert-1);i<num;i++){
    	walls[i]=-(i+1-horiz*(vert-1));
    }
    for(i=num-1;i>=0;i--){
    	int pos=randInt(i+1);
    	int tmp=walls[pos];
    ...
    for(i=0;i<num;i++){
    	...
    	if(sum>=0){
    		x=sum%horiz;
    		y=sum/horiz;
    		int set1=maze.find(sum);
    		int set2=maze.find(sum+horiz);
    		if(set1!=set2){
