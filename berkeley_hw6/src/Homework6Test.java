...
public class Homework6Test {
  ...
  public static void main(String[] args) {
    ...
    table.insert(board, 100);

    SimpleBoard board1 = new SimpleBoard();
    for (int y = 0; y < 8; y++) {
      for (int x = 0; x < 8; x++) {
	int value = 1;
	board1.setElementAt(x, y, value);
      }
    }
    Entry test=table.find(board1);
    if(test!=null){
    System.out.println("find the value is "+test.value());
    }
    System.out.println("size after insert a new board "+table.size());
    
    table.remove(board);

    test=table.find(board1);
    if(test!=null){
    System.out.println("find the value is "+test.value());
    }
    
    System.out.println("size after remove the new board "+table.size());
  }
