...
public class ListSorts {
	...
	public static LinkedQueue makeQueueOfQueues(LinkedQueue q) {
		...
		for (int i = 0; i < length; i++) {
			LinkedQueue unit = new LinkedQueue();
			try {
				unit.enqueue(q.dequeue());
			} catch (QueueEmptyException e) {
				System.err.println("there is a empty exception!");
			}
			out.enqueue(unit);
		}
	...
	public static void quickSort(LinkedQueue q) {
		if (q.size() < 2)
			return;
		Object pivot = q.nth(new Integer((int) (Math.random() * (q.size() - 1) + 1)));
