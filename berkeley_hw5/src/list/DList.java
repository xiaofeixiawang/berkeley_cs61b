...
public class DList extends List {
	 ...
	 * x.next != null. 3) For every DListNode x in a DList, x.prev != null. 4)
	 * For every DListNode x in a DList, if x.next == y, then y.prev == x. 5)
	 * For every DListNode x in a DList, if x.prev == y, then y.next == x. 6)
	 * For every DList l, l.head.myList = null. (Note that l.head is the
	 * sentinel.) 7) For every DListNode x in a DList l EXCEPT l.head (the
