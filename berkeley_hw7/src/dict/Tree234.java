...
public class Tree234 extends IntDictionary {
	...
	public void insert(int key) {
		...
		Tree234Node node = root;
		while (true) {
			if (node.key1 == key || (node.keys > 1 && node.key2 == key) || (node.keys > 2 && node.key3 == key))
				return;
			if (node.keys == 3) {
				Tree234Node pnode = node.parent;
				int tar = node.key2;
				Tree234Node rnode = new Tree234Node(pnode, node.key3);
				rnode.child1 = node.child3;
				rnode.child2 = node.child4;
				if (rnode.child1 != null) {
					rnode.child1.parent = rnode;
				}
				if (rnode.child2 != null) {
					rnode.child2.parent = rnode;
				}
				if (pnode == null) {
					pnode = new Tree234Node(null, node.key2);
					rnode.parent = pnode;
					pnode.child1 = node;
					pnode.child2 = rnode;
					node.parent = pnode;
					root = pnode;
				} else if (pnode.child1 == node) {
					pnode.keys++;
					pnode.key3 = pnode.key2;
					pnode.key2 = pnode.key1;
					pnode.key1 = node.key2;
					pnode.child4 = pnode.child3;
					pnode.child3 = pnode.child2;
					pnode.child2 = rnode;
				} else if (pnode.child2 == node) {
					pnode.keys++;
					pnode.key3 = pnode.key2;
					pnode.key2 = node.key2;
					pnode.child4 = pnode.child3;
					pnode.child3 = rnode;
				} else {
					pnode.keys++;
					pnode.key3 = node.key2;
					pnode.child4 = rnode;
				}
				node.keys = 1;
				node.child3 = null;
				...
				if (node.child1 == null)
					break;
				node = node.child1;
			} else if (node.keys == 1 || node.key2 > key) {
				if (node.child2 == null)
					break;
				node = node.child2;
			} else {
